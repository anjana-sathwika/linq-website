...
function Login() {
  const { signedIn, pendingProfile, signInWithGoogle } = useStore();
  const navigate = useNavigate();

  useEffect(() => {
    if (signedIn) navigate({ to: "/search" });
    else if (pendingProfile) navigate({ to: "/onboarding" });
  }, [signedIn, pendingProfile, navigate]);
...
function GoogleIcon() {
  return (
    <svg className="size-5" viewBox="0 0 48 48" aria-hidden>
      ...
      <path fill="#4CAF50" d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2C29.2 35 26.7 36 24 36c-5.2 0-9.6-3.1-11.3-7.5l-6.5 5C9.6 39.6 16.3 44 24 44z"/>
      <path fill="#1976D2" d="M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.3 4.2-4.1 5.6l6.2 5.2C41 35.4 44 30.1 44 24c0-1.2-.1-2.4-.4-3.5z"/>
    </svg>
  );
}
